...
class ImageApiGenerator(ImageGeneratorBase):
    ...
    def _normalize_resolution(self, resolution: str) -> str:
        """标准化分辨率格式为小写（如 4k, 2k, 1k）"""
        if not resolution:
            return "2k"
        # 转换为小写并移除空格
        normalized = resolution.lower().replace(" ", "").replace("K", "k")
        # 确保以k结尾
    ...
    def _generate_via_images_api(
        ...
        """通过 /v1/images/generations 或 /v1/images/compositions 端点生成图片"""
        headers = {
        ...
        if all_reference_images:
            logger.debug(f"  使用图生图模式，添加 {len(all_reference_images)} 张参考图片")
            ...
            payload = {
                ...
                "images": image_uris,
                "ratio": aspect_ratio,
                "resolution": self._normalize_resolution(self.image_size)
            }
        ...
        logger.debug(f"  API 响应: data 长度={len(data_list) if data_list else 0}")
        ...
        if data_list and len(data_list) > 0:
            ...
            if "b64_json" in item:
                ...
                # 处理 URL 格式的响应
                image_url = item["url"]
                logger.info(f"✅ 从 Image API 获取到图片 URL: {image_url}")
